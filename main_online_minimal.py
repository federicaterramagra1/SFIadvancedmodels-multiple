...
def run_statistical_srs_campaign(
    ...
    pilot   = pilot   if pilot   is not None else getattr(SETTINGS, "STAT_PILOT", 1000)
    ...
    block   = block   if block   is not None else getattr(SETTINGS, "STAT_BLOCK", 200)
    ...
    needed = max(0, n_target - n_inj)
    gen2 = srs_combinations(all_faults, r=N, seed=seed+1, max_yield=needed if needed > 0 else None)

    block_acc = 0
    pbar2 = tqdm(total=needed if needed > 0 else None, desc=f"[STAT] N={N} target~{n_target} (eps_ci={eps_ci}, conf={conf})")
    for combo in gen2:
        inj_id += 1
        frcrit, faults, bias, fh, mbc, cbc, cm = _evaluate_combo(
            model, device, test_loader, clean_by_batch, injector, combo, inj_id, total_samples,
...
def run_fault_injection(
    ...
    if force_exhaustive or total_possible <= MAX_FAULTS:
        ...
        avg_frcrit, ci, n_used, top_sorted, out_file = run_statistical_srs_campaign(
            ...
            block=getattr(SETTINGS, "STAT_BLOCK", 200),
...
if __name__ == "__main__":
    ...
    # Sweep: ESAUSTIVA garantita per N <= 3, altrimenti STAT con CI
    for N in [ 1104, 800, 900, 1000, 1200, 1400, 1600, 1700, 1800, 2000, 2100, 2200]:
        run_fault_injection(
            model=model,
            device=device,
            ...
            N=N,
            MAX_FAULTS=getattr(SETTINGS, "MAX_FAULTS", 1000),
            save_dir="results_minimal",
            seed=getattr(SETTINGS, "SEED", 0),
            exhaustive_up_to_n=3  # esaustiva forzata fino a N=3
        )
